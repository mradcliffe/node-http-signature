...
module.exports = {
  ...
  parseRequest: function parseRequest(request, options) {
    ...
    assert.object(parsedOptions, 'options');
    assert.optionalFinite(parsedOptions.clockSkew, 'options.clockSkew');
    ...
    if (parsedOptions.headers.length > 0) {
      parsedOptions.headers.forEach((headerName) => {
    ...
    assert.string(request.url, 'request.url');
    ...
    sigs.forEach((sig, sigIndex) => {
      ...
      assert.arrayOfString(sigMatch, `bad signature ${sigIndex} format`);
      if (sigMatch.length !== 3) {
        throw new InvalidHeaderError(`bad signature ${sigIndex} format`);
      }
    ...
    sigInputs.forEach((sigInputString) => {
      ...
      const sigInputParts = sigInputString.match(/^([a-zA-Z0-9-_]+)=(.*)$/);
      assert.arrayOfString(sigInputParts, `bad signature-input format`);
      if (sigInputParts !== null && sigInputParts.length === 3) {
        ...
        inputParams.forEach((inputParam, inputParamIndex) => {
          if (inputParamIndex === 0) {
            ...
            const [paramKey, paramValue] = inputParam.split('=', 1);
        ...
        if (sigParams['created']) {
          ...
          if (created === null ||
            ...
            if (skew > parsedOptions.clockSkew) {
              throw new ExpiredRequestError(`created lies in the future with skew ${skew}s greater than allowed ${parsedOptions.clockSkew}s`);
            }
          }
        }
        ...
        if (sigParams['expires']) {
          ...
          if (expires === null ||
            ...
            throw new InvalidParamsError('expires is not valid')
          } else {
            const expiredSince = Math.floor(now / 1000) - expires;
            if (expiredSince > parsedOptions.clockSkew) {
              throw new ExpiredRequestError(`request expired with skew ${expiredSince}s greater than allowed ${parsedOptions.clockSkew}s`)
            }
          }
        }
        ...
        // Extract the keyid.
        if (sigParams['keyid']) {
          if (!/"[^"]+"/.test(sigParams['keyid'])) {
            throw new InvalidParamsError('bad param format');
          }
          parsedSignature.params.keyId = sigParams['keyid'].replace('"', '');
        } else {
          throw new InvalidParamsError('keyid was not specified');
        }

        // Determine the algorithm.
        if (sigParams['alg']) {
          const { alg } = sigParams;
          if (!parsedOptions.algorithms[alg]) {
            throw new InvalidAlgorithmError(`${alg} is not a valid algorithm`);
          }
          parsedSignature.params.algorithm = alg;
        } else {
          throw new InvalidParamsError('alg was not specified');
        }
